...
async function findHandlerByPathPattern(
  ...
  interface Candidate {
    ...
    isValid: boolean;  // Structural validation passed
  }
      ...
      try {
        ...
        if (rows && rows.length > 0) {
          for (const row of rows) {
            ...
            // STRUCTURAL VALIDATION: Check if path structure matches
            let isValidCandidate = true;
            if (annotationPath) {
              // Normalize paths for structural comparison
              if (!pathsMatchStructurally(pathPattern, annotationPath)) {
              ...
              // No annotation path extracted - cannot validate structure
              // Give a penalty but don't reject outright (might be complex @RequestMapping)
              score -= 200;
            }
            ...
            if (!candidates.some(c => c.handler === handler && c.filePath === filePath)) {
              candidates.push({ handler, filePath, line, content, score, annotationPath, isValid: isValidCandidate });
            }
          }
        }
      } catch (e) {
