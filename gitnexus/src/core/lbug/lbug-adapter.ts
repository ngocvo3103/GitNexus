...
const getCopyQuery = (table: NodeTableName, filePath: string): string => {
  ...
  if (table === 'Route') {
    return `COPY ${t}(id, name, filePath, httpMethod, handler, controller, framework, prefix, lineNumber, responseKeys, errorKeys, middleware) FROM "${filePath}" ${COPY_CSV_OPTS}`;
  }
  if (table === 'Tool') {
