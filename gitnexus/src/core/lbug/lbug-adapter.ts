...
const getCopyQuery = (table: NodeTableName, filePath: string): string => {
  ...
  if (table === 'Method') {
    return `COPY ${t}(id, name, filePath, startLine, endLine, isExported, content, description, parameterCount, returnType, parameters, annotations) FROM "${filePath}" ${COPY_CSV_OPTS}`;
  }
  if (table === 'Class') {
